...
/*
  ── L'avatar accroché au coin de la carte du héros ────────────────────────

  La planche le dessine en cinq blocs pleins : un disque miel, une masse de
  cheveux prune, un visage, deux yeux, un sourire. Pas d'image à charger, pas de
  photo de personne réelle à faire signer : c'est un pictogramme, et il pèse
  zéro octet de réseau.

  ⚠️ DEUX ENCRES SONT DES ENCRES D'ILLUSTRATION, pas des jetons d'interface.
  Le teint et le trait des yeux ne figurent pas dans la palette et n'ont pas à y
  figurer : un jeton d'interface se réutilise, une carnation ne se réutilise
  nulle part ailleurs. Elles sont donc nommées ici, à l'endroit unique où elles
  servent, plutôt que glissées dans les jetons où elles deviendraient une
  couleur de marque que personne n'a ratifiée.

  Il est décoratif de bout en bout : `aria-hidden`, et invisible sous `md` où la
  carte prend toute la largeur et où le débord sortirait de l'écran.
*/
const TEINT = '#8A5A2B'
const TRAIT_DU_VISAGE = '#241209'

export function Avatar() {
  return (
    <div
      aria-hidden
      className="absolute -bottom-[22px] -left-[22px] hidden size-[76px] overflow-hidden rounded-pilule bg-celebration md:block"
    >
      <span className="absolute top-[7px] left-[12px] size-[52px] rounded-pilule bg-prune" />
      <span
        className="absolute top-[24px] left-[19px] h-[40px] w-[38px] rounded-pilule"
        style={{ background: TEINT }}
      />
      <span
        className="absolute top-[41px] left-[29px] size-[5px] rounded-pilule"
        style={{ background: TRAIT_DU_VISAGE }}
      />
      <span
        className="absolute top-[41px] right-[29px] size-[5px] rounded-pilule"
        style={{ background: TRAIT_DU_VISAGE }}
      />
      <span
        className="absolute top-[49px] left-[32px] h-[7px] w-[12px] rounded-b-[12px]"
        style={{ borderBottom: `2px solid ${TRAIT_DU_VISAGE}` }}
      />
    </div>
  )
}
